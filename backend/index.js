...
// // Fetch Google Fitness Data
// app.get("/fetch-data", async (req, res) => {
//   try {
//     const fitness = google.fitness({ version: "v1", auth: oAuth2Client });

//     const { displayName, profilePhotoUrl, userID } = userProfileData;
//     const startTimeMillis = Date.now() - 14 * 24 * 60 * 60 * 1000; 
...
//     saveUserDataToAppwrite({ displayName, profilePhotoUrl, userID });
//     formattedData.forEach(saveFitnessDataToAppwrite);

//     res.send({ displayName, profilePhotoUrl, userID, formattedData });
//   } catch (error) {
//     console.error("Error fetching fitness data:", error);
//     res.redirect("/error");
//   }
...
//   try {
//     const users = await database.listDocuments(process.env.DATABASE_ID, process.env.COLLECTION_ID);
//     if (!users.documents.some((user) => user.profileURL === userData.profilePhotoUrl)) {
//       await database.createDocument(process.env.DATABASE_ID, process.env.COLLECTION_ID, ID.unique(), {
//         username: userData.displayName,
//         profileURL: userData.profilePhotoUrl,
//         userID: userData.userID,
